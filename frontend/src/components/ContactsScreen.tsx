...
type Contact = { id: number; owner_name: string; phone_number: string; created_at: string };
...
export function ContactsScreen() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setError(null);
    try {
      await createContact({ owner_name: ownerName.trim(), phone_number: phoneNumber.trim() });
      setOwnerName("");
      setPhoneNumber("");
      await refresh();
    } catch (err) {
    ...
    <Card className="border-border/80 shadow-none">
      <CardHeader className="flex flex-row items-center justify-between gap-4">
        <div className="space-y-0.5">
          ...
          <div className="text-xs text-muted-foreground">Unique by phone number (duplicates are rejected).</div>
        </div>
      ...
      <CardContent>
        ...
        <div className="mt-6 space-y-2">
          {loading ? (
            ...
            contacts.map((c) => (
              <div key={c.id} className="flex items-center justify-between gap-3 rounded-md border border-border/60 bg-background/70 px-3 py-2">
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium text-foreground">{c.owner_name}</div>
